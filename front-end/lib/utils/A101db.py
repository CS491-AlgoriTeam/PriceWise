...
for currentUrl in final_urls:
    ...
    for product in arrayOfProjectHTML:
        ...
        current_product = Product(current_name, current_price)
        product_final_list.append(current_product)

# Function to push data to Firestore
def push_to_firestore(product_list):
    db = firestore.client()
    products_ref = db.collection('Market').document('A101').collection('Categories').document('Meyve Sebze').collection('Products')
